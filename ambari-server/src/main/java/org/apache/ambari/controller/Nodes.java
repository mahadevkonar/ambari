...
public class Nodes {
     ...
     * TODO: simplify logic? 
     */
    public List<Node> getNodesByState (String allocatedx, String alivex) 
    		throws Exception {
        /*
        ...
        XMLGregorianCalendar curTime = DatatypeFactory.newInstance().
        ...
        for (Node n : this.nodes.values()) {
            ...
            if (allocatedx.equals("") && alive) {
                if (getTimeDiffInMillis(curTime, n.getNodeState().
                		getLastHeartbeatTime()) < NODE_NOT_RESPONDING_DURATION) {
                    list.add(n);
                    continue;
                }
            }
            ...
            if (allocated && alive) {
                if (n.getNodeState().getAllocatedToCluster() && 
                    getTimeDiffInMillis(curTime, n.getNodeState().
                    		getLastHeartbeatTime()) < NODE_NOT_RESPONDING_DURATION) {
                    list.add(n);
                    continue;
                }
    ...
    public Node getNode (String name) throws Exception {
        if (!this.nodes.containsKey(name)) {
            ...
            throw new WebApplicationException((new 
            		ExceptionResponse(msg, Response.Status.NOT_FOUND)).get());
        }
     ...
     * Register new node
     */
    public synchronized void checkAndUpdateNode (String name,
    		Date hearbeatTime) throws Exception {
